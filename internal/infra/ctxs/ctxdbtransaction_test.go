...
func TestContextWithTransaction(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "with-tx-value-1", args: args{ctx: context.Background(), tx: tx1}, want: context.WithValue(context.Background(), xTransactionKey, tx1)},
		{name: "with-tx-value-2", args: args{ctx: context.Background()}, want: context.WithValue(context.Background(), "other-key", tx2)},
		{name: "with-tx-value-3", args: args{ctx: context.Background(), tx: tx3}, want: context.WithValue(context.Background(), xTransactionKey, tx3)},
	}
